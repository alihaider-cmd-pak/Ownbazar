...
if ($bank1.success) {
    ...
    $proofPayload = @{
        orderId = $orderId2
        proofs = @("data:image/png;base64,$proofBase64", "data:image/png;base64,$proofBase64")
    }
...
if ($easy1.success) {
    ...
    $proofPayload = @{
        orderId = $orderId3
        proofs = @("data:image/png;base64,$proofBase64", "data:image/png;base64,$proofBase64", "data:image/png;base64,$proofBase64")
    }
...
Write-Host "`n📋 TEST 4: Verifying data in data/orders.json..." -ForegroundColor Yellow
try {
    $ordersFile = Get-Content "d:\WorkSpace\data\orders.json" -Raw | ConvertFrom-Json
    $lastThree = $ordersFile [-3..-1]
    
    Write-Host "✅ JSON file readable and valid" -ForegroundColor Green
    Write-Host "`n📊 Last 3 Created Orders:" -ForegroundColor Cyan
    $lastThree | ForEach-Object {
        ...
        Write-Host "   $($_.orderNumber) - $($_.paymentMethod.ToUpper()) - $proofCount proof(s)" -ForegroundColor Green
    }
    
    $results += @{
        test = "Data Integrity Check"
        status = "✅ PASS"
        totalOrders = $ordersFile.Count
...
# TEST 5: JazzCash Order
Write-Host "`n📋 TEST 5: Creating JazzCash Order..." -ForegroundColor Yellow
$jazzOrder = @{
    items = $testCart
    subtotal = 2500
    tax = 250
...
$results | ForEach-Object {
    ...
    if ($_.proofs -and $_.proofs -ne "N/A (COD)" -and $_.proofs -ne "Pending") { Write-Host "   Proofs: $($_.proofs)" -ForegroundColor Gray }
    if ($_.error) { Write-Host "   Error: $($_.error)" -ForegroundColor Gray }
    if ($_.totalOrders) { Write-Host "   Total Orders: $($_.totalOrders)" -ForegroundColor Gray }
}
